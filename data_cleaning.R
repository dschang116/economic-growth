...
rGDPcap <- read_excel("shiny_app/raw_data/fred_rgdp_cap.xls") %>% 
  slice(-c(1:10)) %>% 
  rename("rGDPpercap" = ...2) %>% 
...
rGDPcapgrowth <- read_excel("shiny_app/raw_data/fred_rgdp_capgrowth.xls") %>% 
  slice(-c(1:10)) %>% 
...
HHI <- read_excel("shiny_app/raw_data/WITS_HHI.xlsx", 
